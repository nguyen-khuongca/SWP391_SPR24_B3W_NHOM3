...
namespace SWP391_BL3W.Repository
{
    public class BaseRepository<TEntity> : IBaseRepository<TEntity> where TEntity : class
    {
        ...
        public BaseRepository(SWPContext context)
        {
            _context = context;
...
<<<<<<< HEAD
        public async Task<TEntity?> GetByEmailAndPassword(string email, string password)
        {
            // Ensure that TEntity is of type User
            if (typeof(TEntity) != typeof(User))
            {
                throw new InvalidOperationException("GetByEmailAndPassword method is only supported for User entities.");
            }

            // Create a filter expression to match the provided email and password
            Expression<Func<TEntity, bool>> filter = entity =>
                (entity as User).Email == email && (entity as User).Password == password;
            // Call FirstOrDefaultAsync method with the filter expression
